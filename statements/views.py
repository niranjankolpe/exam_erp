...
def labstaff_excel(request):
    if request.method == 'POST':
        ...
        for row in ws.iter_rows(min_row=1, min_col=1, max_col=29):
            for cell in row:
                cell.font = header_font
                cell.alignment = center_alignment

        for row in ws.iter_rows(min_row=4, min_col=1, max_col=29):
            for cell in row:        
...
def internal_external_bill_excel(request):
    if request.method == 'POST':
        college_name = request.POST['college_name']
        university = request.POST['university']
        time_period = request.POST['time_period']
